...
class TaskControllerTest {
    ...
    fun testCreate() {
        Mockito.doReturn(task)
            .`when`(taskService).create(text, estimationTime, taskColor, userId)

        val taskJson = """{"text": "$text", "estimationTime": "$estimationTime", "color": "$color", "userId": "$userId"}"""
    ...
    companion object {
        ...
        val task = Task(id, text, LocalDateTime.now(), estimationTime, taskColor, userId)
    }
}