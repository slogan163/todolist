...
class TaskServiceImpl(private val repository: TaskRepository) : TaskService {
    ...
    override fun create(text: String, estimationDate: LocalDateTime, color: TaskColor, userId: UUID): Task {
        val task = Task(text = text, estimationTime = estimationDate, color = color, userId = userId)
